...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    loss += .5 * self.reg * (np.linalg.norm(self.params['W1'])**2 \
                                + np.linalg.norm(self.params['W2'])**2)  
            
    dx2, dw2, grads['b2'] = affine_backward(dx, cache2)
    dx1, dw1, grads['b1'] = affine_relu_backward(dx2, cache1)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(1, self.num_layers):
        if self.use_batchnorm:
            out, caches[i] = affine_batchnorm_relu_forward(\
                               out, self.params['W'+str(i)],\
                               self.params['b'+str(i)], self.params['gamma'+str(i)],\
                               self.params['beta'+str(i)], self.bn_params[i-1])
        else:
            out, caches[i] = affine_relu_forward(out, self.params['W'+str(i)], self.params['b'+str(i)])
        if self.use_dropout:
    ...
    loss, dx = softmax_loss(scores, y) 
    loss += .5 * self.reg * \
            sum(np.sum(self.params['W'+str(i)]**2) for i in xrange(1, self.num_layers + 1))
...
def affine_batchnorm_relu_forward(x, w, b, gamma, beta, bn_param):
  """
  Helper layer that performs an affine transform followed by a Batchnorm 
  and a ReLU

  Inputs:
      - x: Input to the affine layer
      - w, b: Weights for the affine layer
      - gamma, beta: Weights for batch normalization layer
